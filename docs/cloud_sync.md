...
历史列表最多加载 100 个版本。“查看备份”立即打开独立内容窗口，展示时间、可搜索的文件名/提示词/标签与恢复统计；收藏原图按需加载预览（超过 32 MiB 时仅显示文件信息）。关闭窗口不应用备份，只有单独选择“恢复到本机”才写入本地。界面恢复使用 localOnly journal，预览确认后仅应用所选历史到本地，远端 HEAD 不变；手动模式也可使用。保留应用前恢复点用于失败回滚，应用前后仍复核远端 revision。
