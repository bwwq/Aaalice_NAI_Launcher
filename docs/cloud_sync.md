...
- 加密对象映射可带 `partMetadata`，逐密文分卷保存大小及 `verificationRevision`。证明绑定后端、对象位置和已校验版本，不含凭据或凭据派生值；与 HEAD 条件提交使用的 revision 分开。跨设备读取索引后，通过对象清单或 HEAD 确认全部分卷仍存在且版本未变，即可复用内容，无需下载、解密原有 payload。ETag 仅作为版本验证器，不替代 SHA-256。
- 旧 v4 索引缺少分卷元数据时完整读取校验，并在下一份索引中补齐；不改写历史版本。远端版本变化只重新校验对应分卷，缺失对象由本地内容补传；缺少可靠版本验证器的服务继续读取校验。恢复下载仍验证密文和明文的 SHA-256。
- 小对象打包以上一次已提交索引为基线，保留成员、顺序、大小及打包资格均未变的包，仅重新打包其余内容。待续传操作优先使用持久化分组和原密文映射，不随新基线重排；缺失必要的续传密文时报告失败，不提交不完整版本。
...
| S3 | AWS SigV4；支持路径式/虚拟主机式地址，ListObjectsV2 分页。对象复用通过 HEAD 比较大小及已校验的版本 ID/强 ETag；HEAD 不支持或验证器缺失时读取校验。默认区域 us-east-1；保存连接仅验证读取。保持手动备份、单并发，写后读回校验，不承诺原子条件提交；旧备份与整个命名空间需在服务控制台删除 |
| WebDAV | 根据实际 ETag/条件写能力决定模式；对象强 ETag 复用与 HEAD 条件提交能力分别判断，手动模式也可复用已验证且版本未变的对象。能力不足保持手动备份；坚果云单并发，不自动合并或恢复历史 |
...
启动时立即开始恢复已保存连接，账号信息先于网络校验显示，首次恢复期间不显示重复登录入口；无已保存连接时不得重建正在编辑的设置表单。已连接时的前台刷新保留 Dashboard，首次恢复只读取一次 HEAD，成功后清除先前失败状态。自动上传可按本机连接独立启用；自动拉取仍未启用。
...
仅验证版本索引增量传输时，使用 `-IndexedIncremental`。该模式在五类后端的本地 HTTP 模拟适配器分别处理 64 MiB 内容，分阶段验证首次上传、同机/重启/空缓存设备的重复备份、修改一条 1 MiB 记录及完整恢复；分别记录内容与索引/元数据的请求数、读写字节和阶段耗时。单阶段保留 30 秒测试上限，操作超时会取消并等待结束后再清理缓存；不会运行默认综合测试集。功能与恢复回归入口为 `indexed_backup_transfer_test.dart`、`indexed_backup_packing_test.dart`、`indexed_backup_recovery_test.dart`。
...
历史列表最多加载 100 个版本。“查看备份”立即打开独立内容窗口，只读取随上传保存的加密清单及 metadata 对象/pack，展示时间与可搜索的文件名/提示词/标签，不下载原图、不扫描本机图库；schema 2/3 的旧清单也使用同样的按需读取。选择“检查恢复影响”后才下载完整备份、扫描本机并展示变化统计，随后再次确认才应用；收藏原图按需加载预览（超过 32 MiB 时仅显示文件信息）。关闭窗口不应用备份，只有单独选择“恢复到本机”才写入本地。界面恢复使用 localOnly journal，预览确认后仅应用所选历史到本地，远端 HEAD 不变；手动模式也可使用。保留应用前恢复点用于失败回滚，应用前后仍复核远端 revision。

每个本机连接独立保存自动备份偏好，不参与同步，默认全部关闭。可指定每日本地时间，或启用最后一次变动后等待 1–1440 分钟。连续修改重置等待期；定时到期也不能越过正在等待的变动延迟。仅在软件运行且连接就绪、没有预览/冲突/其他操作时执行上传；失败至少等待 5 分钟重试，可取消待执行项。关闭软件期间不执行，重开后按下次每日时间运行；待变动备份保存在本地，过期待执行项重新留出完整等待时间。

设置、词库与收藏等已打开 Hive 数据源和本地图库修改产生事件；恢复写入排除在事件触发外。触发上传时再次比较本地内容与同步基线，内容未变则不创建快照。没有统一事件源的外部文件编辑（例如用其他程序改 Skill 文件或原图）需由定时/手动备份覆盖。原图验证、备份提交、加密及保留策略与手动上传相同。
